...
export function decideOutcome(rtp, betAmount, isFreeSpin, gameId) {
  ...
  const winChanceMult = gameId === 'fullhouse' ? 0.24 : (gameId === 'wild-bounty' ? (isFreeSpin ? 0.27 : 0.31) : (gameId === 'gates-of-olympus' ? 0.22 : (gameId === 'argonauts' ? 0.07 : (gameId === 'big-brown' ? 0.19 : (gameId === 'thimbles' ? 0.75 : (gameId === 'hi-lo' ? 0.92 : (gameId === 'mines' ? 0.82 : 0.15)))))));
  const winChance = rtpFrac * winChanceMult;
  const isWin = Math.random() < winChance;
  if (!isWin) {
