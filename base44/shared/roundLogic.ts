...
export function decideOutcome(rtp, betAmount, isFreeSpin, gameId) {
  ...
  const winChanceMult = gameId === 'fullhouse' ? 0.20 : (gameId === 'wild-bounty' ? 0.22 : (gameId === 'gates-of-olympus' ? 0.22 : (gameId === 'argonauts' ? 0.07 : (gameId === 'big-brown' ? 0.19 : (gameId === 'thimbles' ? 0.75 : (gameId === 'hi-lo' ? 0.75 : (gameId === 'mines' ? 0.75 : 0.15)))))));
  const winChance = rtpFrac * winChanceMult;
  const isWin = Math.random() < winChance;
  if (!isWin) {
