...
import { getWgGame } from '@/lib/wgGames';
...
// Launches a WG game: our backend creates the seamless-wallet session and
// returns the signed game URL, which we render full screen.
export default function WgGame() {
  const { gameId } = useParams();
  const [url, setUrl] = useState('');
  const [error, setError] = useState('');
  const [progress, setProgress] = useState(0);
  const [ready, setReady] = useState(false);
  const title = getWgGame(gameId)?.name || 'WG';
  useGameFullscreen();
