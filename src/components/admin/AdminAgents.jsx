...
export default function AdminAgents() {
  ...
  const loadAgents = () => agentOps('agents').then(r => setAgents(r.agents || []));
  ...
  const setRole = async (role) => {
    const res = await agentOps('set_role', { q: found?.id || q.trim(), role });
    if (!res.ok) { setMsg(agentError(res)); return; }
    setFound(res.user);
    setMsg(role === 'agent' ? `${res.user.username} is now an agent.` : `${res.user.username} is no longer an agent.`);
    setKey(k => k + 1);
  };
      ...
      <div className="dash-card p-4 flex flex-col gap-3">
        ...
        {found && (
          <div className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(212,175,55,0.25)' }}>
            ...
            {found.role === 'agent' ? (
              <button onClick={() => setRole('user')} className="px-3 py-2 rounded-lg text-[12px] font-bold flex items-center gap-1"
                style={{ border: '1px solid rgba(248,113,113,0.5)', background: 'rgba(248,113,113,0.12)', color: '#f87171' }}>
                <UserMinus className="w-4 h-4" /> Remove
              </button>
            ) : (
              <button onClick={() => setRole('agent')} className="dash-btn-gold px-3 py-2 text-[12px] flex items-center gap-1">
