...
export default function SuperAceMachine() {
  ...
  const doSpin = async () => {
    ...
    // Anticipation: 2 early scatters → remaining columns slow-mo.
    ...
    // Nudge the grid to match the server's win/loss decision BEFORE showing
    // it, so the cards drop once with their final faces — no face change.
    {
    ...
    // Golden Wild (applied before showing the grid, uses flip animation later)
    ...
    // Show the final grid and start the spin animation — cards drop once with
  ...
  return (
    ...
    >
      ...
      <main className="max-w-none mx-auto px-2 py-2">
        ...
        <div className="w-full rounded-2xl relative p-[3px]" style={{ background: 'transparent', border: '3px solid #c8932e', boxShadow: '0 0 0 2px #2e1d0a, 0 0 0 4px rgba(200,150,60,0.4), 0 16px 48px rgba(0,0,0,0.75)' }}>
          <div className="flex flex-col gap-2 rounded-[13px] overflow-hidden relative" style={{ background: 'transparent' }}>
            ...
            <div className="px-2 py-3 border-t" style={{ background: 'transparent', borderTop: '1px solid rgba(190,140,55,0.5)' }}>
