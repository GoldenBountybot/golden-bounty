...
export default function SuperAceMachine() {
  ...
  const doSpin = async () => {
    ...
    // No tease/anticipation slow-mo — all columns drop at normal speed so 1-2
    // scatters never cause a long pause.
    setTeaseStart(-1);
    setTeaseCols(new Set());
    const baseSpin = turboRef.current ? 600 : 1000;
    const spinDur = baseSpin;

    // Start the spin animation immediately — cards drop with their faces.
    // No nudge, no card changes. The grid is exactly as generated by makeGrid.
    setSpinning(true);
    setGrid(g.map((c) => ({ ...c })));
