...
export default function SuperAceMachine() {
  ...
  const doSpin = async () => {
    ...
    const forceWin = Math.random() < Math.min(0.3, Math.max(0.05, (rtpRef.current / 100) - 0.45));
  ...
  const resolveCascades = async (g) => {
    ...
    while (true) {
      ...
      const stopChance = inFreeRef.current
        ? [0, 0.62, 0.82, 0.93][Math.min(comboCount, 3)] || 0.98
        : [0, 0.72, 0.88, 0.96][Math.min(comboCount, 3)] || 0.99;
  ...
  return (
    ...
    >
      ...
      <main className="max-w-md lg:max-w-xl mx-auto px-2 py-2">
