...
export default function SuperAceMachine() {
  ...
  const doSpin = async () => {
    ...
    const forceWin = Math.random() < Math.min(0.85, (rtpRef.current / 100) - 0.15);
  ...
  const resolveCascades = async (g) => {
    ...
    while (true) {
      ...
      const stopChance = inFreeRef.current
        ? [0, 0.16, 0.32, 0.50][Math.min(comboCount, 3)] || 0.70
        : [0, 0.26, 0.46, 0.66][Math.min(comboCount, 3)] || 0.85;
