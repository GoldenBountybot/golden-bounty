...
export default function GatesMachine() {
  ...
  }, [stoppedReels, grid, dropCells, g.tumbleSeq, turbo]);

  return (
    <div ref={machineRef} className="relative w-full max-w-none mx-auto flex flex-col overflow-hidden select-none"
                          ...
                          {stopped ? (
                            <div key={animKey} className="relative w-full h-full"
                              style={{ animation: isScatterGlow ? 'gatesScatterGlow 0.9s ease-in-out infinite'
                                ...
                                :                                 dropAnim ? `gatesDrop ${g.turbo ? 0.42 : 0.64}s cubic-bezier(0.33,0.04,0.5,1) both` : 'none',
                                willChange: (isShatter || isWin || isScatterGlow || dropAnim) ? 'transform, opacity' : 'auto',
                                backfaceVisibility: 'hidden',
                                transform: (isShatter || isWin || isScatterGlow || dropAnim) ? 'translateZ(0)' : 'none' }}>
