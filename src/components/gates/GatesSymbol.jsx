...
export default function GatesSymbol({ sym, highlight }) {
  if (isMult(sym)) {
    ...
    return (
      <div className="w-full h-full flex items-center justify-center relative rounded-[5px] overflow-hidden"
        ...
        <img src={SYM_IMG.mult} alt={`mult-${v}`} className="w-full h-full object-contain" style={{ filter: 'drop-shadow(0 0 4px rgba(100,220,50,0.6)) saturate(1.25) contrast(1.12) brightness(1.05)', mixBlendMode: 'lighten' }} />
  ...
  return (
    <div className="w-full h-full flex items-center justify-center relative rounded-[5px] overflow-hidden"
      ...
      {img ? (
        <img src={img} alt={sym} className="w-full h-full object-contain" style={{ imageRendering: 'auto', mixBlendMode: 'lighten', filter: 'saturate(1.25) contrast(1.12) brightness(1.05)' }} />
      ) : (
