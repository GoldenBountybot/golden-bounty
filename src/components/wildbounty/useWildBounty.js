...
export function useWildBounty() {
  ...
  const evaluateAndCascade = (currentGrid, cascadeCount, totalWin, currentMultIndex, wasFree, scatterAwarded = false, framedPositions = new Set()) => {
    ...
    if (stepWin > 0 && serverWinRef.current > 0) {
      ...
      // History is logged authoritatively by the settleBet backend function —
      // no client-side logging here (it caused duplicate history rows).
  ...
  const spin = useCallback(async () => {
    ...
    if (roll < 0.0003) targetScatters = 3;             // 0.03% (free-spin trigger)
    else if (roll < 0.008) targetScatters = 2;          // 0.77%
    else if (roll < 0.07) targetScatters = 1;          // 6.2%
