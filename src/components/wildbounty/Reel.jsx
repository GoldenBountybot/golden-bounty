  ...
  return (
    <div className={`relative w-full ${spinning ? 'overflow-hidden' : 'overflow-visible'}`} style={{ aspectRatio: '1 / ' + rowCount }}>
      ...
      <div
        ...
        style={{ animation: spinning ? `reelFall ${speed}s linear infinite` : justStopped ? (wasAnticipation.current ? 'reelLandSlow 1.1s ease-out' : 'reelLand 0.4s ease-out') : 'none', willChange: spinning ? 'transform' : 'auto', filter: spinning ? (anticipationGlow ? 'blur(1.5px) saturate(1.1)' : 'blur(1px)') : 'none' }}
      >
        {strip.map((sym, i) => {
