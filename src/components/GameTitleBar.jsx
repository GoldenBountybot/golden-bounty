...
export default function GameTitleBar({ title, icon, left, right, padLeft = 'pl-24', padRight = 'pr-24', maxWidth = 'max-w-md' }) {
  return (
    <div className={`${maxWidth} mx-auto px-3 py-2.5 relative`}>
      ...
      {left && <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center">{left}</div>}
      {right && <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5">{right}</div>}
    </div>
  );
}